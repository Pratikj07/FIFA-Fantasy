...
export default function Home() {
  ...
  const liveMatches  = allResolved.filter(m => m.status === 'LIVE');
  const ftMatches    = allResolved.filter(m => m.status === 'FT').slice(-3);
  const nextMatch    = allResolved.find(m => m.status === 'UPCOMING' && !predictions[m.id]);
  ...
  // Real rank from Supabase — count users with more points than me
  ...
  useEffect(() => {
    ...
    const fetchRank = async () => {
      if (!supabase) {
        setRankData({ rank: 1, total: 1, loaded: true });
        return;
      }
      try {
        // Fetch all predictions + total user count in parallel
        ...
        // Count how many OTHER users have strictly more points than me
    ...
    fetchRank();
  }, [user?.id, myPts, completedResults]);
