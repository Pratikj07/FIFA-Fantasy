...
// Parse "Jun 11" + "3:00 PM" → sortable Date
const parseDateTime = (date, time) => {
  const months = {Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11};
  const [mon, day] = date.split(' ');
  const m = time.match(/(\d+):(\d+)\s*(AM|PM)/i);
  if (!m) return new Date(2026, months[mon], parseInt(day));
  let [, h, min, ap] = m;
  h = parseInt(h); min = parseInt(min);
  if (ap.toUpperCase() === 'PM' && h !== 12) h += 12;
  if (ap.toUpperCase() === 'AM' && h === 12) h = 0;
  return new Date(2026, months[mon], parseInt(day), h, min);
};

// All 72 matches sorted chronologically once at module level
const SORTED_MATCHES = [...MATCHES].sort(
  (a, b) => parseDateTime(a.date, a.time) - parseDateTime(b.date, b.time)
);

// Matchday groups
const BY_MATCHDAY = SORTED_MATCHES.reduce((acc, m) => {
...
// Today's date string matching matches format e.g. "Jun 12"
...
export default function Predictions() {
  ...
  const todayCount    = SORTED_MATCHES.filter(m => m.date === today).length;
  const visibleTotal  = SORTED_MATCHES.filter(filterMatch).length;
  ...
  return (
    <>
      <div className="space-y-5">
        ...
        <div className="flex gap-2 flex-wrap">
          {[
            { val:'TODAY',          label:`📅 Today · ${today}`, badge: todayCount > 0 ? todayCount : null },
            { val:'UPCOMING',       label:'Upcoming' },
            { val:'ALL',            label:'All Matches' },
            { val:'MY PREDICTIONS', label:`My Picks (${predCount})`, icon: true },
        ...
        {/* TODAY tab — daily matches only */}
        {tab === 'TODAY' && (
          visibleTotal === 0 ? (
            <div className="text-center py-16 text-white/30 space-y-3">
              <Calendar size={40} className="mx-auto opacity-20"/>
              <p className="font-semibold text-white/40">No matches today</p>
              <p className="text-sm">Check back on matchdays or switch to Upcoming</p>
            </div>
          ) : (
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <span className="font-display text-xl text-wc-gold tracking-wider">Today · {today}</span>
                <div className="flex-1 h-px bg-white/5"/>
                <span className="text-white/30 text-[10px]">{visibleTotal} matches</span>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <AnimatePresence mode="popLayout">
                  {SORTED_MATCHES.filter(filterMatch).map((m, i) => (
        ...
        {/* Other tabs — grouped by Matchday */}
        {tab !== 'TODAY' && (
          visibleTotal === 0 ? (
            <div className="text-center py-16 text-white/30">
              ...
              <p className="text-sm mt-1">
                {tab === 'MY PREDICTIONS' ? 'No predictions yet — go to Upcoming to start' : 'Try a different filter'}
              </p>
            </div>
          ) : (
            [1, 2, 3].map(md => {
              const mdMatches = (BY_MATCHDAY[md] || []).filter(filterMatch);
        ...
        <div className="glass-card p-4 text-center">
          <p className="text-white/30 text-xs">
            💡 Today shows only today's matches. Upcoming shows all future matches in schedule order.
            Tap <strong className="text-wc-blue">AI</strong> for instant match analysis.
          </p>
        </div>
      </div>
